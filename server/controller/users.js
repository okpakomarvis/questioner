  ...
  comment: (req, res) => {
    ...
    db.query("SELECT * FROM questions where question_id = $1", [
      ...
      .then(result => {
        ...
        db.query(
          "INSERT INTO comments(questions, comment) VALUES($1, $2) RETURNING *",
          [req.body.question, req.body.comment.trim()]
        )
          .then(data => {
            if (data.rowCount < 1) {
              return res.json({
  ...
  createRvsp: (req, res) => {
    ...
    db.query("select * from meetups where meetup_id = $1", [meetup_id])
      .then(data => {
        ...
        return res.json({
          status: 200,
          error: " You Have Already Join This Meetup "
        });
      })
      .catch(e => {
        if (e.name === "error") {
    ...
    const { userId } = req.userData;
    const queryrsvp =
      'INSERT INTO rsvp("user" , meetup, response) VALUES($1, $2, $3) RETURNING *';
    db.query(queryrsvp, [userId, meetup_id, response.trim()])
      .then(data => {
        if (data.rowCount < 1) {
          return res.json({
            status: 404,
